...
class LibraryActions {
  ...
  Future<ImportResult> importBook(String sourcePath) async {
    ...
    final row = BooksCompanion.insert(
      ...
      title: title,
      subtitle: Value(probe.subtitle),
      author: Value(author.isEmpty ? null : author),
      publisher: Value(probe.publisher),
      language: Value(probe.language),
      series: Value(probe.series),
      seriesIndex: Value(probe.seriesIndex),
      description: Value(probe.description),
      tagsJson: Value(jsonEncode(probe.tags)),
      localPath: Value(localPath),
      coverPath: Value(coverPath),
      coverHash: Value(coverHash),
      coverSource: const Value(0),
      fileSize: Value(fileSize),
      totalChars: Value(probe.totalChars),
      addedAt: existing?.addedAt ?? now,
      updatedAt: now,
      hlc: hlc.encode(),
      updatedBy: deviceId,
      deleted: const Value(false), // 复活墓碑：用户重新导入就是要它回来
    );
    ...
    if (existing.isEmpty) {
      ...
      for (final r in RulePresets.all) {
        await db.into(db.rules).insertOnConflictUpdate(RulesCompanion.insert(
              ...
              colorValue: r.colorValue,
              bgColorValue: r.bgColorValue,
              bgOpacity: Value(r.bgOpacity),
              bold: Value(r.bold),
              italic: Value(r.italic),
              underline: Value(r.underline),
              priority: Value(r.priority),
              scopeCsv: Value(r.scope.map((s) => s.name).join(',')),
