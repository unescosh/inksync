...
/// 冲突合并 —— 与 Rust 侧 `core/src/sync/merge.rs` 是**同一套语义的两份实现**。
///
/// ⚠️ 改本文件必须同步改 Rust 那份，反之亦然。两边一旦语义分叉，就是最难查的
/// 一类 bug：同一条变更在手机端与 UOS 端合并出不同结果，不报错，只在事后表现为
/// 跨端数据不一致。改动时请显式比对另一侧的用例。
///
/// 注：Rust 那份目前没有生产消费者（无头 CLI 按设计只做二进制备份/恢复，见
/// docs/09 第 3 节的分工），它的消费者是「未来的 Rust SyncEngine」（docs/02 §5）。
///
